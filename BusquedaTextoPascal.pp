...
uses
  SysUtils,
  Classes, //libreria para utilizar la clase TStringList
  DateUtils, //libreria para utilizar TDateTime
  StrUtils, //libreria para utilizar pos y posEx
  crt; //libreria para utilizar clrscr y readkey
...
procedure VerApariciones(textoSeleccionado, palabraBuscada: string);
    ...
    cantidadApariciones, i, opcion, posicionActual: Integer;
    oracion, palabraResaltada: string;
    begin
    cantidadApariciones := 0;
    posicionActual := 0;

    // Encontrar todas las apariciones de la palabra buscada y guardar sus posiciones
    SetLength(posiciones, 0);
    i := Pos(palabraBuscada, textoSeleccionado);
    while i > 0 do
    begin
        Inc(cantidadApariciones);
        SetLength(posiciones, cantidadApariciones);
        posiciones[cantidadApariciones - 1] := i;
        i := PosEx(palabraBuscada, textoSeleccionado, i + 1);
    end;

    // Mostrar el resultado
    if cantidadApariciones > 0 then
    begin
        repeat
        oracion := Copy(textoSeleccionado, Max(1, posiciones[posicionActual] - 50), 100);
        palabraResaltada := Copy(oracion, posiciones[posicionActual] - Max(1, posiciones[posicionActual] - 50) + 1, Length(palabraBuscada));
        oracion := StringReplace(oracion, palabraResaltada, '[' + palabraResaltada + ']', [rfReplaceAll]);
        writeln(oracion);

        // Mostrar el menú de opciones
        writeln('Opciones:');
        writeln('1. Siguiente aparicion');
        writeln('2. Aparicion anterior');
        writeln('0. Cancelar operacion');
        ...
        case opcion of
            ...
            begin
            Inc(posicionActual);
            if posicionActual >= cantidadApariciones then
                posicionActual := 0;
            end;
            2:
            begin
            ...
            if posicionActual < 0 then
                posicionActual := cantidadApariciones - 1;
            end;
            0: Break;
        else
